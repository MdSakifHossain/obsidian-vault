...
To stop this `systemd` from running after the next boot, we can just swap out the eable to disable. Example:

```bash
systemctl --user disable autolock-session.service
```

and **Reboot**, it will not run after system boots next time.
